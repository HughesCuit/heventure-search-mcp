...
class BenchmarkResults:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "成功率": f"{(self.success_count / (self.success_count + self.error_count) * 100):.2f}%"
            ...
            "95百分位响应时间": f"{statistics.quantiles(self.response_times, n=20)[18]:.2f}ms"
            if len(self.response_times) >= 20
            else "N/A",
            "99百分位响应时间": f"{statistics.quantiles(self.response_times, n=100)[98]:.2f}ms"
            if len(self.response_times) >= 100
            else "N/A",
            "QPS": f"{self.success_count / total_time:.2f}"
            if total_time > 0
            else "N/A",
            "平均内存使用": f"{statistics.mean(self.memory_usage):.2f}MB"
            if self.memory_usage
            else "N/A",
            "峰值内存使用": f"{max(self.memory_usage):.2f}MB"
            if self.memory_usage
            else "N/A",
            "平均CPU使用率": f"{statistics.mean(self.cpu_usage):.2f}%"
            if self.cpu_usage
            else "N/A",
...
class MCPBenchmark:
    ...
    async def run_sequential_benchmark(self, test_count: int = 50) -> BenchmarkResults:
        ...
        for i in range(test_count):
            ...
            if i % 2 == 0:
                ...
                print(
                    f"搜索测试 {i + 1}/{test_count}: {query[:20]}... - {response_time:.2f}ms - {'成功' if success else '失败'}"
                )
            else:
                url = self.test_urls[i % len(self.test_urls)]
                response_time, success = await self.benchmark_single_content_fetch(url)
                print(
                    f"内容测试 {i + 1}/{test_count}: {url} - {response_time:.2f}ms - {'成功' if success else '失败'}"
                )
    ...
    async def run_concurrent_benchmark(
        self, concurrent_count: int = 10, requests_per_worker: int = 5
    ) -> BenchmarkResults:
        """运行并发基准测试"""
        print(
            f"开始并发基准测试 ({concurrent_count} 并发, 每个worker {requests_per_worker} 请求)..."
        )
        results = BenchmarkResults()
        results.start_time = time.time()
            ...
            for i in range(requests_per_worker):
                ...
                if (worker_id * requests_per_worker + i) % 2 == 0:
                    query = self.test_queries[
                        (worker_id * requests_per_worker + i) % len(self.test_queries)
                    ]
                    response_time, success = await self.benchmark_single_search(query)
                    print(
                        f"Worker {worker_id} 搜索 {i + 1}: {response_time:.2f}ms - {'成功' if success else '失败'}"
                    )
                else:
                    url = self.test_urls[
                        (worker_id * requests_per_worker + i) % len(self.test_urls)
                    ]
                    response_time, success = await self.benchmark_single_content_fetch(
                        url
                    )
                    print(
                        f"Worker {worker_id} 内容 {i + 1}: {response_time:.2f}ms - {'成功' if success else '失败'}"
                    )
...
async def main():
    ...
    try:
        ...
        print("- 建议并发限制: 5-10个并发请求")
        print("- 建议请求频率: 每秒不超过10个请求")
